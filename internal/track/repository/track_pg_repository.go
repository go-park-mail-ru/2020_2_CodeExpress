...
func (ar *TrackRep) SelectByArtistIdWithFav(artistId uint64, userId uint64) ([]*models.Track, error) {
...
func (ar *TrackRep) SelectByArtistId(artistId uint64) ([]*models.Track, error) {
	rows, err := ar.dbConn.Query("SELECT tracks.id, tracks.album_id, tracks.title, tracks.duration, tracks.index, tracks.audio, albums.poster, artists.id, artists.name FROM tracks JOIN albums ON tracks.album_id = albums.id JOIN artists ON albums.artist_id = artists.id WHERE artists.id = $1;",
		artistId)
	if err != nil {
		return nil, err
	}
...
func (ar *TrackRep) SelectByParamsWithFav(count uint64, from uint64, userId uint64) ([]*models.Track, error) {
	...
	for rows.Next() {
		...
		var userFavouriteId sql.NullInt64
		if err := rows.Scan(&track.ID, &track.AlbumID, &track.Title, &track.Duration, &track.Index, &track.Audio,
			&track.AlbumPoster, &track.Artist, &track.ArtistID, &userFavouriteId); err != nil {
			return nil, err
		}
...
func (ar *TrackRep) SelectByParams(count uint64, from uint64) ([]*models.Track, error) {
	rows, err := ar.dbConn.Query("SELECT tracks.id, tracks.album_id, tracks.title, tracks.duration, tracks.index, tracks.audio, albums.poster, artists.id, artists.name FROM tracks JOIN albums ON tracks.album_id = albums.id JOIN artists ON albums.artist_id = artists.id ORDER BY tracks.title LIMIT $1 OFFSET $2;",
		count, from)
	if err != nil {
		return nil, err
	}
